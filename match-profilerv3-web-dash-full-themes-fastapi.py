...
import time # Added for cache buster

# --- FastAPI Imports ---
from fastapi import FastAPI, Request # Request might be needed for templates if using Jinja2Templates engine
from fastapi.responses import HTMLResponse, RedirectResponse
from fastapi.middleware.cors import CORSMiddleware
import uvicorn
import jinja2 # For rendering the HTML string with Jinja syntax
...
FASTAPI_PORT: int = 8881 # Changed from FLASK_PORT
FASTAPI_DEBUG: bool = False # Uvicorn handles debug/reload separately
...
app = FastAPI(title="Matchmaking Dashboard Avançado - FastAPI", version="1.3")
...
# --- HTML Template (Mantido EXATAMENTE como na versão anterior) ---
# (Omitido por brevidade - Cole o conteúdo completo da variável index_html aqui)
index_html = """
<!DOCTYPE html>
<html lang="pt-BR">
<head>
...
<body class="min-h-screen"> <!-- Theme class added by JS -->
    ...
    <script>
        ...
        function loadTheme() {
            ...
            const themeToApply = savedTheme && themes[savedTheme] ? savedTheme : '{{ DEFAULT_THEME }}'; // Use Jinja default
                    ...
                    .map(([key, value]) => `
                        <li>
                            ...
                            <span class="font-semibold score-detail-value">${parseFloat(value).toFixed(2)}</span>
                             <span class="text-[0.65rem] score-detail-key">(Peso: ${WEIGHTS[key] ? WEIGHTS[key].toFixed(2) : '0.00' })</span>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // --- Handle Initial Data State (Passed from Python/Jinja) ---
            // Need to use a safe way to parse JSON embedded in JS
            let initialError = null;
            let initialDataLoaded = false;
            let initialOriginProfile = null;
            let initialSimilarProfiles = [];
            try {
                initialError = JSON.parse('{{ error_message | tojson | safe }}');
                initialDataLoaded = JSON.parse('{{ data_loaded_completely | tojson | safe }}');
                initialOriginProfile = JSON.parse('{{ perfil_origem | tojson | safe }}');
                initialSimilarProfiles = JSON.parse('{{ perfis_similares | tojson | safe }}');
            } catch (e) {
                 console.error("Error parsing initial data from Jinja:", e);
                 initialError = "Erro ao carregar dados iniciais do servidor.";
            }
            ...
            if (retestBtn) {
                retestBtn.addEventListener('click', () => {
                    ...
                    // Redirect to the new match route (FastAPI)
                    window.location.href = '/new_match'; // Relative URL should work
                    // No need to handle response here, the page will reload
                });
            }
        });

        // --- Global Constants (from Jinja) ---
        // Ensure these are correctly passed and parsed
        let WEIGHTS = {};
        let MIN_REQUIRED_PLATFORM_SCORE = 0.0;
        let MIN_REQUIRED_AVAILABILITY_SCORE = 0.0;
        try {
             WEIGHTS = JSON.parse('{{ WEIGHTS | tojson | safe }}');
             MIN_REQUIRED_PLATFORM_SCORE = JSON.parse('{{ MIN_REQUIRED_PLATFORM_SCORE | tojson | safe }}');
             MIN_REQUIRED_AVAILABILITY_SCORE = JSON.parse('{{ MIN_REQUIRED_AVAILABILITY_SCORE | tojson | safe }}');
        } catch(e) {
             console.error("Error parsing constants from Jinja:", e);
        }

        // Fix for score details rendering in JS (needs access to WEIGHTS, etc.)
        // The previous Jinja rendering within the JS block needs to be replaced
        // because Jinja runs server-side only once. We make WEIGHTS etc. global JS vars.
         function renderSimilarProfiles(profiles) { // RE-DEFINE to use JS globals
            const section = document.getElementById('similarProfilesSection');
            const grid = document.getElementById('similarProfilesGrid');
            const noResultsMsg = document.getElementById('noSimilarProfilesMessage');
            const title = document.getElementById('similarProfilesTitle');
            ...
            profiles.forEach((perfil, index) => {
                const cardDiv = document.createElement('div');
                cardDiv.className = `profile-card rounded-lg p-4 flex flex-col h-full fade-in similar-card-${index}`;

                // Use JS variables WEIGHTS, MIN_REQUIRED_PLATFORM_SCORE, etc. here
                const scoreDetailsHtml = Object.entries(perfil.score_details || {})
                    .map(([key, value]) => {
                        const weight = WEIGHTS[key] !== undefined ? WEIGHTS[key].toFixed(2) : 'N/A';
                        const platformWarning = (key === 'plataformas' && value < MIN_REQUIRED_PLATFORM_SCORE)
                            ? '<i class="fas fa-exclamation-triangle text-yellow-500 ml-1" title="Abaixo do threshold mínimo"></i>' : '';
                        const availabilityWarning = (key === 'disponibilidade' && value < MIN_REQUIRED_AVAILABILITY_SCORE)
                            ? '<i class="fas fa-exclamation-triangle text-yellow-500 ml-1" title="Abaixo do threshold mínimo"></i>' : '';

                        return `<li>
                            <span class="inline-block w-24 score-detail-key">${key.charAt(0).toUpperCase() + key.slice(1)}:</span>
                            <span class="font-semibold score-detail-value">${parseFloat(value).toFixed(2)}</span>
                            <span class="text-[0.65rem] score-detail-key">(Peso: ${weight})</span>
                            ${platformWarning}
                            ${availabilityWarning}
                         </li>`;
                    })
                ...
                // ... (rest of the innerHTML definition, using scoreDetailsHtml and contatoHtml) ...
                 cardDiv.innerHTML = `
                    <div class="flex justify-between items-start mb-2">
                        <h4 class="text-lg font-bold font-orbitron text-gradient-dynamic flex-grow pr-2">${perfil.nome} <span class="text-xs font-light">(ID: ${perfil.id})</span></h4>
                        <div class="text-right flex-shrink-0">
...
# --- Configuração do Jinja2 para renderizar a string HTML ---
jinja_env = jinja2.Environment(
    loader=jinja2.BaseLoader(), # Não carrega de arquivos
    autoescape=jinja2.select_autoescape(['html', 'xml']),
    # Adiciona um filtro personalizado para formatar float (se necessário em outros lugares)
    # filters={'format': lambda value, format_spec: format(value, format_spec)} # Não usado diretamente no template, mas pode ser útil
)
# Adiciona funções e variáveis globais ao ambiente Jinja
jinja_env.globals['safe_split_and_strip'] = safe_split_and_strip
jinja_env.globals['WEIGHTS'] = WEIGHTS
jinja_env.globals['MIN_REQUIRED_PLATFORM_SCORE'] = MIN_REQUIRED_PLATFORM_SCORE
jinja_env.globals['MIN_REQUIRED_AVAILABILITY_SCORE'] = MIN_REQUIRED_AVAILABILITY_SCORE
jinja_env.globals['DEFAULT_THEME'] = DEFAULT_THEME
jinja_env.globals['available_themes'] = AVAILABLE_THEMES
# jinja_env.globals['url_for'] = app.url_path_for # Se precisar gerar URLs FastAPI no template (não é o caso aqui)

# Carrega o template a partir da string
index_template = jinja_env.from_string(index_html)

# --- Rota FastAPI Principal ---
...
async def read_index():
    ...
    template_data = {
        ...
        "perfil_origem": None,
        "perfis_similares": [],
        # Globals são definidos no jinja_env, não precisam ser passados aqui novamente
        # "available_themes": AVAILABLE_THEMES,
        # "DEFAULT_THEME": DEFAULT_THEME,
        "current_year": datetime.datetime.now().year,
        "id_origem": None, # Será definido se buscarmos um perfil
        # "WEIGHTS": WEIGHTS, # Passado via jinja_env.globals
        # "MIN_REQUIRED_PLATFORM_SCORE": MIN_REQUIRED_PLATFORM_SCORE, # Passado via jinja_env.globals
        # "MIN_REQUIRED_AVAILABILITY_SCORE": MIN_REQUIRED_AVAILABILITY_SCORE, # Passado via jinja_env.globals
    }
    ...
    if not app_data["data_loaded"] or app_data["loading_error"]:
        ...
        html_content = index_template.render(template_data)
...
@app.get("/new_match", response_class=RedirectResponse)
async def new_match():
    ...
    # Usa um timestamp
    cache_buster = f"?t={time.time()}"
    # O URL para a rota raiz é simplesmente "/"
    return RedirectResponse(url=f"/{cache_buster}", status_code=302) # 302 Found é padrão para redirects


# --- Função para iniciar o carregamento em background (usada no startup) ---
...
# --- Evento de Startup do FastAPI usando o novo sistema de eventos ---
from contextlib import asynccontextmanager

@asynccontextmanager
async def lifespan(app: FastAPI):
    """Função executada quando o servidor FastAPI inicia e termina."""
    # Startup
    ...
    if not db_profiles_exists or not db_embeddings_exists:
        ...
        # Define o erro no estado da aplicação para que a rota raiz possa mostrá-lo
        app_data["loading_error"] = f"{msg} Profile DB exists: {db_profiles_exists}, Embedding DB exists: {db_embeddings_exists}. Check logs."
        # Não podemos sair do processo aqui, mas o erro será registrado e exibido
    else:
        logging.info("Database files found.")
        print("⏳ Carregando dados e construindo índice (isso pode levar alguns minutos)...")
        # Carrega os dados diretamente, sem background
        load_data_and_build_index()
        if app_data["data_loaded"]:
            print("✅ Dados carregados com sucesso!")
        else:
            print("❌ Erro ao carregar dados. Verifique os logs para mais detalhes.")
...
if __name__ == '__main__':
    # A verificação de banco de dados agora acontece no evento lifespan
    # Apenas imprimimos informações sobre como iniciar
    print(f"\n🚀 Iniciando servidor Uvicorn na porta {FASTAPI_PORT}...")
    print(f"   Acesse o dashboard em: http://127.0.0.1:{FASTAPI_PORT}")
    print(f"   Logs detalhados em: {LOG_FILE_VAL}")
    print("   Pressione CTRL+C para parar o servidor.")

    # Usar Uvicorn para rodar a aplicação FastAPI
    # reload=False é importante para não reiniciar o processo e perder o estado/background task
    # use_colors=True pode ser útil no terminal
    uvicorn.run(
        "__main__:app",  # Referencia o objeto 'app' neste arquivo
        host="127.0.0.1",
        port=FASTAPI_PORT,
        reload=False, # Mantenha False para produção e para o background thread funcionar corretamente
        log_level="info", # Nível de log fixo para o uvicorn
    )